SELECT data.project_id, repo.repo_name, repo.url, source_environment.url AS environment_url, data.file, data.later_date, MAX(earlier_commits.commit_date) AS earlier_date
FROM (
...
LEFT JOIN gros.source_environment ON data.project_id = source_environment.project_id AND repo."type" = source_environment.source_type
WHERE data.file NOT LIKE '%/pom.xml' AND data.file <> 'pom.xml' ${category_conditions}
GROUP BY data.project_id, repo.repo_name, repo.url, source_environment.url, data.file, data.later_date
