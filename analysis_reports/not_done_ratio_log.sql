...
FROM
(
    SELECT CAST(log(${sprint_points_normalization}*CAST(issue.story_points AS double)/CAST(CASE WHEN sprint_points.max_points IS NOT NULL AND sprint_points.max_points <> 0 THEN sprint_points.max_points ELSE ${sprint_points_normalization} END AS double)) AS DECIMAL(5,2)) AS num_points, COUNT(*) AS num_not_done FROM gros.issue,
...
FULL OUTER JOIN
(
    SELECT CAST(log(${sprint_points_normalization}*CAST(issue.story_points AS double)/CAST(CASE WHEN sprint_points.max_points IS NOT NULL AND sprint_points.max_points <> 0 THEN sprint_points.max_points ELSE ${sprint_points_normalization} END AS double)) AS DECIMAL(5,2)) AS num_points, COUNT(*) AS num_done FROM gros.issue,
    (SELECT issue_id, MIN(changelog_id) AS first_changelog_id FROM gros.issue
        WHERE type = 7
		AND issue.story_points <> 0
		AND issue.sprint_id <> 0
