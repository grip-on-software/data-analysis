SELECT COALESCE(COALESCE(not_done.num_points, done.num_points), 0) AS story_points, not_done.num_not_done, done.num_done, COALESCE(not_done.num_not_done,0)/CAST(COALESCE(not_done.num_not_done,0)+COALESCE(done.num_done,0) AS double)*100 AS ratio, COALESCE(done.num_done,0)/CAST(COALESCE(not_done.num_not_done,0)+COALESCE(done.num_done,0) AS double)*100 AS done_ratio
FROM
(
    ...
    (SELECT issue.issue_id, max_issue.changelog_id, MIN(sprint.sprint_id) AS late_sprint FROM gros.issue, gros.sprint,
            (SELECT issue_id, sprint_id, MAX(changelog_id) AS changelog_id FROM gros.issue
                GROUP BY issue_id, sprint_id) AS max_issue
            WHERE issue.sprint_id = sprint.sprint_id
            AND issue.issue_id = max_issue.issue_id
            AND ${issue_not_done}
            AND ${issue_overdue} AND issue.changelog_id = max_issue.changelog_id
            GROUP BY issue.issue_id, max_issue.changelog_id) AS not_done_data,
