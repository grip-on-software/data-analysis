SELECT project."name", cast(weekdata.end_date as date) AS sprint_end, weekdata.sprint_id, CAST(SUM(story_points) AS float)/(days/7*5) AS velocity
...
GROUP BY project."name", sprint_end, weekdata.sprint_id, days
ORDER BY project."name", sprint_end;
