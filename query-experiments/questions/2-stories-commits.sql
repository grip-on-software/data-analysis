SELECT num_stories, num_commits FROM
(SELECT project_id, sprint_id, COUNT(*) AS num_stories FROM
    (SELECT DISTINCT project_id, type, issue_id, story_points, sprint_id FROM gros.issue) AS latest_issues
    WHERE sprint_id <> 0 AND type = 7
