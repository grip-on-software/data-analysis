...
(SELECT DISTINCT project, sprint, date, SUM(num_points) AS num_open FROM
    (SELECT project_id AS project, issue_id, sprint_id AS sprint, MAX(story_points) AS num_points, MIN(updated) AS date FROM gros.issue WHERE type = 7 AND status = 3 GROUP BY project_id, issue_id, sprint_id
    UNION ALL
    SELECT project_id AS project, issue_id, sprint_id AS sprint, MAX(story_points) AS num_points, MIN(updated) AS date FROM gros.issue WHERE type = 7 AND resolution <> 0 GROUP BY project_id, issue_id, sprint_id) AS updatedata,
    ...
    (SELECT project_id, sprint_id, issue_id, MIN(updated) AS end_date FROM gros.issue WHERE type = 7 AND resolution <> 0 GROUP BY project_id, sprint_id, issue_id) AS enddata
