...
collect <- function() {
	...
	query <- "SELECT project.project_id, project.name, sprint.sprint_id, sprint.start_date, comment.message FROM gros.comment JOIN gros.issue ON issue.issue_id = comment.issue_id AND issue.changelog_id = 0 JOIN gros.project ON issue.project_id = project.project_id JOIN gros.sprint ON sprint.project_id = project.project_id AND comment.date BETWEEN sprint.start_date AND ${sprint_close} WHERE project.is_support_team = false AND message NOT LIKE 'Versie%\nGeslaagd:%' AND message <> '*Resolving* as a result of the *Resolve* action being applied to the parent.' ORDER BY project.name, sprint.sprint_id"
...
projects <- lapply(split(res, res$name), function(project) {
	values <- aggregate(list(polarity=as.numeric(as.character(project$polarity))),
						list(start_date=as.Date(project$start_date, '%Y-%m-%d'), sprint_id=project$sprint_id),
						mean)
...
sentiment_data <- do.call(rbind, projects)[,c('project_id', 'sprint_num', 'polarity')]
